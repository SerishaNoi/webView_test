...
class DragGesturePullToRefresh extends VerticalDragGestureRecognizer {
  ...
  FixedScrollMetrics _getMetrics(double minScrollExtent, double maxScrollExtent, double pixels,
  ...
  DragGesturePullToRefresh() {
    onStart = (DragStartDetails dragDetails) {
      ...
      if (!isLoading || (msLoading > 0 && (DateTime.now().millisecondsSinceEpoch - msLoading) > EXCEEDS_LOADING_TIME)) {
    ...
    onUpdate = (DragUpdateDetails dragDetails) {
      if (dragStarted) {
        ...
        ScrollUpdateNotification(
                metrics: _getMetrics(dy > 0 ? 0 : dragDistance, height, dy > 0 ? (-1) * dy : dragDistance, height,
    ...
    onEnd = (DragEndDetails dragDetails) {
      ScrollEndNotification(
              metrics: _getMetrics(0, height, dragDistance, height, AxisDirection.down), context: _context)
          .dispatch(_context);
      _clearDrag();
    };
    onCancel = () {
      ScrollUpdateNotification(
              metrics: _getMetrics(0, height, 1, height, AxisDirection.up), context: _context, scrollDelta: 0)
          .dispatch(_context);
      _clearDrag();
    };
  }
